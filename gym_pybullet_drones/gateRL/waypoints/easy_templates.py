import numpy as np
from .WaypointTemplate import *

class OneForwardTemplate(WaypointTemplate):
    def __init__(self,):
        waypoints_xyzs = np.array([
            np.array([0, 0, 0]),
            np.array([1, 0, 0]),
        ])  
        waypoints_rpys = np.array([
            np.array([0, 0, 0]),
            np.array([0, 0, 0]),
        ])
        spawns = [
            {
                "pos": np.array([-1.1, 0, 0]),
                "vel": np.array([0, 0, 0]),
                "acc": np.array([0, 0, 0]),
                "rpy": np.array([0, 0, 0]),
                "next_waypoints": [0, 1]
            }
        ]
        waypoints_normal_distr = np.array([
            np.array([[0, 0], [0, 0], [0, 0]]),
            np.array([[0, 0], [0, 1], [0, 1]]),
        ])
        waypoints_rpys_choices = [
            [RPY_FRONT_UP],
            [RPY_FRONT_UP, RPY_LEFT_UP, RPY_RIGHT_UP, RPY_BACK_UP, RPY_DOWN_BACK, RPY_DOWN_LEFT, RPY_DOWN_RIGHT],
        ]
        waypoints_scale = [0.5, 2.0]
        max_dist = 8

        super().__init__(
            waypoints_xyzs=waypoints_xyzs,
            waypoints_rpys=waypoints_rpys,
            spawns=spawns,
            waypoints_normal_distr=waypoints_normal_distr,
            rpy_choices=waypoints_rpys_choices,
            waypoints_scale=waypoints_scale,
            max_dist=max_dist,
            difficulty="easy"
        )

class OneBackTemplate(WaypointTemplate):
    def __init__(self,):
        waypoints_xyzs = np.array([
            np.array([0, 0, 0]),
            np.array([-1, 0, 0]),
        ])  
        waypoints_rpys = np.array([
            np.array([0, 0, 0]),
            np.array([0, 0, 0]),
        ])
        spawns = [
            {
                "pos": np.array([-1.1, 0, 0]),
                "vel": np.array([0, 0, 0]),
                "acc": np.array([0, 0, 0]),
                "rpy": np.array([0, 0, 0]),
                "next_waypoints": [0, 1]
            }
        ]
        waypoints_normal_distr = np.array([
            np.array([[0, 0], [0, 0], [0, 0]]),
            np.array([[0, 0], [0, 1], [0, 1]]),
        ])
        waypoints_rpys_choices = [
            [RPY_FRONT_UP],
            [RPY_FRONT_UP, RPY_LEFT_UP, RPY_RIGHT_UP, RPY_BACK_UP, RPY_DOWN_BACK, RPY_DOWN_LEFT, RPY_DOWN_RIGHT],
        ]
        waypoints_scale = [0.5, 2.0]
        max_dist = 8

        super().__init__(
            waypoints_xyzs=waypoints_xyzs,
            waypoints_rpys=waypoints_rpys,
            spawns=spawns,
            waypoints_normal_distr=waypoints_normal_distr,
            rpy_choices=waypoints_rpys_choices,
            waypoints_scale=waypoints_scale,
            max_dist=max_dist,
            difficulty="easy"
        )

class UpDownTemplate(WaypointTemplate):
    def __init__(self,):
        waypoints_xyzs = np.array([
            np.array([0, 0, 0]),
            np.array([0, 0, 0]),
        ])  
        waypoints_rpys = np.array([
            np.array([0, 0, 0]),
            np.array([0, 0, 0]),
        ])
        spawns = [
            {
                "pos": np.array([-1.1, 0, 0]),
                "vel": np.array([0, 0, 0]),
                "acc": np.array([0, 0, 0]),
                "rpy": np.array([0, 0, 0]),
                "next_waypoints": [0, 1]
            }
        ]
        waypoints_normal_distr = np.array([
            np.array([[0, 0], [0, 0], [0, 0]]),
            np.array([[0, 1], [0, 1], [0, 2]]),
        ])
        waypoints_rpys_choices = [
            [RPY_FRONT_UP],
            [RPY_FRONT_UP, RPY_LEFT_UP, RPY_RIGHT_UP, RPY_BACK_UP, RPY_DOWN_BACK, RPY_DOWN_LEFT, RPY_DOWN_RIGHT],
        ]
        waypoints_scale = [0.5, 2.0]
        max_dist = 8

        super().__init__(
            waypoints_xyzs=waypoints_xyzs,
            waypoints_rpys=waypoints_rpys,
            spawns=spawns,
            waypoints_normal_distr=waypoints_normal_distr,
            rpy_choices=waypoints_rpys_choices,
            waypoints_scale=waypoints_scale,
            max_dist=max_dist,
            difficulty="easy"
        )